...
infoToString (((r,c),Empty):xs) board = (case checkAdjacents (r,c) (makeInfo board) of
                                          0 -> ' '
                                          x  -> intToDigit x ) : infoToString xs board


dimensions :: [[a]] -> (Int, Int)
dimensions [] = (0,0)
dimensions board@ (row:_) = (length board, length row)         